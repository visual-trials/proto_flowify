...
/*
    ...
    my.drawContainer = function (camera, sliceContainer, drawingAfterConnections) {
        ...
        var containerBorderNormalizedWidth = containerProperties.containerBorderNormalizedWidth
        var containerTextNormalizedFontHeight = containerProperties.containerTextNormalizedFontHeight
        var containerTextCenterHorizontally = containerProperties.containerTextCenterHorizontally
        var containerTextCenterVertically = containerProperties.containerTextCenterVertically
        var headerTextCenterHorizontally = containerProperties.headerTextCenterHorizontally
        var headerTextCenterVertically = containerProperties.headerTextCenterVertically
        ...
        // FIXME: somehow align with pixels... console.log('nh:' + sliceContainer.worldContainer.normalizedSize.height + 'h:' + sliceContainer.size.height)

        var centerPosition = ZUI.world.getCenterPositionBasedOnPointsTo(absolutePosition, sliceContainer.size, positionPointsTo)
        var x = centerPosition.x - width / 2
        var y = centerPosition.y - height / 2
        ...
        if (showContainerBody) {
            ...
            var borderWidth = 2
            if (containerBorderNormalizedWidth != null) {
                borderWidth = containerBorderNormalizedWidth * (sliceContainer.size.height / sliceContainer.worldContainer.normalizedSize.height)  // FIXME: we probably want to do this in world.js!
            }
        ...
        var iconWidthIncludingMargin = 0

        if (containerProperties.showContainerIcon) {

            //
            // Draw the icon
            //

            var containerIconColor = containerProperties.containerIconColor
            var containerIconBorderColor = containerProperties.containerIconBorderColor
            var containerIconShape = containerProperties.containerIconShape

            // TODO: what should these be?
            var iconHeight = 16  // heightPx * 0.10 ?
            var iconWidth = 16

            var borderWidth = 2

            var iconVerticalMargin = (height - iconHeight) / 2
            var iconHorizontalMargin = iconVerticalMargin + 6 // TODO: maybe independent? Larger?

            iconWidthIncludingMargin = iconHorizontalMargin + iconWidth

            my.drawIcon(
                camera,
                containerIconShape,
                x + iconHorizontalMargin,
                y + iconVerticalMargin,
                iconWidth,
                iconHeight,
                containerIconColor,
        ...
        if (containerProperties.showContainerText) {
            ...
            if (containerTextNormalizedFontHeight != null) {
                fontHeight = containerTextNormalizedFontHeight * (sliceContainer.size.height / sliceContainer.worldContainer.normalizedSize.height)  // FIXME: we probably want to do this in world.js!
            }

            if (!containerTextCenterVertically) {
                labelVerticalMargin = fontHeight // FIXME: what should we use here?
                labelHorizontalMargin = labelVerticalMargin
            }

            var containerTextOpacity = sliceContainer.containerTextOpacity

            var keyValueLabel = sliceContainer.worldContainer.containerData.label

            var textRectangleHeight = height - 2 * labelVerticalMargin
            var textRectangleWidth = width - iconWidthIncludingMargin - 2 * labelHorizontalMargin
            var textRectangleX = x + iconWidthIncludingMargin + labelHorizontalMargin
            var textRectangleY = y + labelVerticalMargin

            var cutOffTextWhenNoMoreRoom = containerProperties.containerTextCutOffTextWhenNoMoreRoom
            var wrapTextWhenNoMoreRoom = containerProperties.containerTextWrapTextWhenNoMoreRoom
            ...
            my.drawTextInRect(
                ...
                textRectangleWidth,
                textRectangleHeight,
                containerTextCenterHorizontally,
                containerTextCenterVertically,
                fontHeight,
                fontColor,
                baseFont,
                opacity * containerTextOpacity,
                cutOffTextWhenNoMoreRoom, 
                wrapTextWhenNoMoreRoom
            )
        ...
        if (containerProperties.showHeader) {
            ...
            var containerHeaderColor = {r: 200, g: 200, b: 200, a: 0.15}
            var containerHeaderBorderColor = {r: 200, g: 200, b: 200, a: 0.15}

            if (containerProperties.containerHeaderColor) {
                containerHeaderColor = containerProperties.containerHeaderColor
                containerHeaderBorderColor = containerProperties.containerHeaderBorderColor
            }

            {
                ...
                // TODO: should the HeaderHeight not be in the header? (which should be a container by itself!)
                //       keep in mind though that the connections also want to know this (attaching to the middle of a header)
                var headerHeight = ZUI.world.getContainerHeaderHeight(sliceContainer)
                var borderWidth = 2
                ...
                if (containerProperties.showHeaderIcon) {

                    //
                    // Draw the icon (inside the header border)
                    //

                    var containerHeaderIconColor = containerProperties.headerIconColor
                    var containerHeaderIconBorderColor = containerProperties.headerIconBorderColor
                    var containerHeaderIconShape = containerProperties.headerIconShape

                    // TODO: what should these be?
                    var iconHeight = 32
                    var iconWidth = 32

                    var iconVerticalMargin = (headerHeight - iconHeight) / 2
                    var iconHorizontalMargin = iconVerticalMargin + 6 // TODO: maybe independent? Larger?
                    ...
                    my.drawIcon(
                        camera,
                        containerHeaderIconShape,
                        x + iconHorizontalMargin,
                        y + height - headerHeight + iconVerticalMargin,
                        iconWidth,
                        iconHeight,
                        containerHeaderIconColor,
                        containerHeaderIconBorderColor,
                        borderWidth,
                        opacity
                    )
                ...
                if (containerProperties.showHeaderText) {

                    //
                    // Draw header text (next to the icon, inside the header border)
                    //

                    var containerLabel = sliceContainer.worldContainer.containerData.label

                    // var baseFont = '"Helvetica Neue",Helvetica,Arial,sans-serif'
                    // var baseFont = 'Verdana,sans-serif'
                    var baseFont = 'Arial'
                    var fontColor = {r: 100, g: 100, b: 100, a: 0.7}

                    // TODO: should this be calculated differently?
                    var labelTextHeight = 16

                    var labelVerticalMargin = (headerHeight - labelTextHeight) / 2
                    var labelHorizontalMargin = labelVerticalMargin + 6 // TODO: maybe independent? Larger?

                    var textRectangleX = x + iconWidthIncludingMargin + labelHorizontalMargin
                    var textRectangleY = y + height - headerHeight + labelVerticalMargin
                    var textRectangleWidth = width - iconWidthIncludingMargin - 2 * labelHorizontalMargin
                    var textRectangleHeight = headerHeight - 2 * labelVerticalMargin

                    // FIXME: this is only the case if the text is one line and fills the vertical height of the header...
                    var fontHeight = headerHeight - 2 * labelVerticalMargin

                    var cutOffTextWhenNoMoreRoom = containerProperties.headerTextCutOffTextWhenNoMoreRoom
                    var wrapTextWhenNoMoreRoom = containerProperties.headerTextWrapTextWhenNoMoreRoom
                    ...
                    my.drawTextInRect(
                        ...
                        textRectangleWidth,
                        textRectangleHeight,
                        headerTextCenterHorizontally,
                        headerTextCenterVertically,
                        fontHeight,
                        fontColor,
                        baseFont,
                        opacity,
    ...
    my.drawContainers = function (camera, currentSliceContainer, drawingAfterConnections) {
        var containerProperties = currentSliceContainer.worldContainer.containerProperties
        ...
        if (currentSliceContainer.isVisible) {
            if (containerProperties.drawContainer) {
                my.drawContainer(camera, currentSliceContainer, drawingAfterConnections)
            }
            ...
            if (currentSliceContainer.hasOwnProperty('sliceChildren')) {
                for (var childIndex = 0; childIndex < currentSliceContainer.sliceChildren.length; childIndex++) {
                    ...
                    my.drawContainers(camera, childSliceContainer, drawingAfterConnections)
                }
            }
        }
